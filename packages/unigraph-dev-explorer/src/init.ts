...
export function init(hostname?: string) {
  ...
  const hst =
    hostname ||
    (window.location.hostname.length ? window.location.hostname : 'localhost');
  ...
  window.unigraph.subscribe(
    ...
    (newBacklinks: any[]) => {
      const newVal = Object.fromEntries(
        JSON.parse(JSON.stringify(newBacklinks)).map((el: any) => [el.uid, el]),
      );
      newBacklinks
        .map((el) => el.uid)
        .map((el) => {
          const subs = window.unigraph.getState('registry/backlinksCallbacks')
            .value[el];
